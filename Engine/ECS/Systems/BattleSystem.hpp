...
public:
	...
	void update(Coordinator* coordinator, Entity* player, Entity* enemy, sf::RenderWindow* window,
		...
		, bool fighting, std::list<Entity*>* living, std::list<Entity*>::iterator current_hero, std::list<Entity*> action_queue
	,BattleGUI* battle_gui, sf::Vector2f mousePosWiew, bool* action_avaible, const float& dt, sf::Texture* background)
	{
		...
		if (fighting)
		{
			...
			else
			{
				...
				animator->update(coordinator, dt);
	...
	void initialize_battle(Coordinator* coordinator, Entity* player, Entity* enemy, std::map<int, Entity>* heroes, std::map<int, Ability>* abilities, std::list<Entity*>* living,
		bool fighting, sf::RenderWindow* window, sf::Font* font, sf::Texture* background_texture, std::list<Entity*> action_queue)
	{
